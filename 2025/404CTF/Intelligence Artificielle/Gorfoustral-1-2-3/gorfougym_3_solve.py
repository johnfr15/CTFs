...
def load_model(
    ...
    if from_ is not None:
        model.load_state_dict(t.load(from_, map_location=device), strict=False)
...
def get_flag(
    ...
    size = random.randint(1, len(perturbated_flag) - 1)

    return "".join(perturbated_flag)[:size]
    ...
    def __init__(
        ...
        for _ in range(size):
            if len(self.examples) < flag_count:
                ...
                if flag.startswith(false_flag):
                    self.examples.append({"user": false_flag, "assistant": "True"})
                else:
                    self.examples.append({"user": false_flag, "assistant": ""})
...
def train(
    ...
    for epoch in range(num_epochs):
        total_loss = 0.0
        print("len batch", len(train_loader))
        for batch_idx, batch in enumerate(train_loader):
            ...
            if train_config.save_every and batch_idx % train_config.save_every == 0:
                save_path = f"./model_epoch{epoch + 1}_batch{batch_idx}.pt"
